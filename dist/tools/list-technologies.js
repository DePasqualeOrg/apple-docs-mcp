 ...
 */
export async function handleListTechnologies(category, language, includeBeta = true, limit = API_LIMITS.DEFAULT_TECHNOLOGIES_LIMIT) {
    try {
        ...
        const data = await httpClient.getJson(APPLE_URLS.TECHNOLOGIES_JSON);
 ...
 */
function parseTechnologies(data) {
    const groups = [];
    if (data.sections) {
 ...
 * Apply filters
 */
function applyTechnologyFilters(groups, filters) {
    ...
    if (filters.limit !== undefined && filters.limit >= 0) {
        ...
        }).filter((group) => group !== null && group.technologies.length > 0);
    }
 ...
 */
function formatTechnologiesList(groups) {
    if (groups.length === 0) {
    ...
    const betaTechs = groups.reduce((sum, group) => sum + group.technologies.filter(tech => tech.tags.includes('Beta')).length, 0);
...
//# sourceMappingURL=list-technologies.js.map