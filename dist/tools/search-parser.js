 ...
 */
function formatSearchUnavailable(query, searchUrl) {
 ...
 */
export async function fetchAppleDocsSearch(query, filterType = 'all') {
    const searchUrl = `${APPLE_URLS.SEARCH}?q=${encodeURIComponent(query)}`;
    try {
        const data = await httpClient.postJson(SEARCH_API_URL, {
            text: query,
            targetResultLocale: 'en',
        });
        const results = mapSearchResults(data, filterType).slice(0, API_LIMITS.MAX_SEARCH_RESULTS);
        return {
