 ...
 */
export async function handleGetTechnologyOverviews(category, searchQuery, includeSubcategories = true, limit = 50) {
    try {
        ...
        const [overviewsData, overviewsIndex] = await Promise.all([
            httpClient.getJson(APPLE_URLS.TECHNOLOGY_OVERVIEWS_JSON),
            httpClient.getJson(APPLE_URLS.TECHNOLOGY_OVERVIEWS_INDEX_JSON),
        ]);
 ...
 */
function parseOverviews(overviewsData, overviewsIndex) {
    const overviews = [];
 ...
 */
function processIndexSection(sections, overviews, depth, parentSection) {
    ...
    sections.forEach(section => {
        ...
        const existingOverview = overviews.find(o => o.url.includes(section.path) || o.title === section.title);
        if (existingOverview) {
 ...
 */
function applyOverviewsFilters(overviews, filters) {
    ...
    if (filters.searchQuery) {
        ...
        filtered = filtered.filter(overview => overview.title.toLowerCase().includes(queryLower) ||
 ...
 */
function formatOverviewsList(overviews) {
 ...
 */
function groupOverviewsByCategory(overviews) {
...
//# sourceMappingURL=get-technology-overviews.js.map