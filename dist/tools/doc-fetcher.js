...
import { formatDocumentHeader, formatDocumentAbstract, formatPlatformAvailability, formatSeeAlsoSection, isSpecificAPIDocument, } from './doc-formatter.js';
/**
 * Format JSON documentation content with enhanced analysis
 */
export function formatJsonDocumentation(jsonData, originalUrl, options = {}) {
    let content = '';
 ...
 */
function formatSpecificAPIContent(jsonData) {
    let content = '';
 ...
 */
function formatAPICollectionContent(jsonData) {
    let content = '';
 ...
 * other framework.
 */
function identifierToDocUrl(identifier, jsonData) {
 ...
 */
function formatTopicSections(jsonData) {
 ...
 */
export async function fetchAppleDocJson(url, options = {}, maxDepth = 2) {
    try {
        ...
        const jsonData = await httpClient.getJson(jsonApiUrl);
 ...
 */
function extractRelatedApis(jsonData) {
    const relatedApis = [];
 ...
 */
function extractReferences(jsonData) {
    const references = [];
        ...
        for (const [, ref] of refEntries) {
            references.push({
                ...
                abstract: ref.abstract
                    ? ref.abstract.map((a) => a?.text ?? '').join(' ').trim()
                    : undefined,
            });
        }
    }
 ...
 */
function extractSimilarApis(jsonData) {
    const similarApis = [];
    // From topicSections
 ...
 */
function analyzePlatformCompatibility(jsonData) {
    ...
    const betaPlatforms = platforms.filter((p) => p.beta).map((p) => p.name).filter((name) => name !== undefined);
    const deprecatedPlatforms = platforms.filter((p) => p.deprecated).map((p) => p.name).filter((name) => name !== undefined);
    return {
        supportedPlatforms,
        betaPlatforms,
        deprecatedPlatforms,
 ...
 */
function formatRelatedApisSection(relatedApis) {
    let content = '\n## Related APIs\n\n';
 ...
 */
function formatSimilarApisSection(similarApis) {
    let content = '\n## Similar APIs\n\n';
    // Group by category
    const groupedApis = {};
 ...
 */
function formatPlatformAnalysisSection(analysis) {
